...
{
    public class MeterRunningTimeInboundMessageParserOnePointZero : IMeterRunningTimeInboundMessageParser
    {
        ...
        public MeterRunningTimeInboundMessage ParseMessage(string messageString)
        {
            try
            {
                ...
                var cumulativeRunningTimeString = tokens[CumulativeRunningTimeIndex];
                var cumulativeRunningTime = TimeSpan.FromMilliseconds(long.Parse(cumulativeRunningTimeString));

                return new MeterRunningTimeInboundMessage
                {
                    MeterNo = meterNo,
                    DcuNo = dcuNo,
                    Timestamp = timestamp,
